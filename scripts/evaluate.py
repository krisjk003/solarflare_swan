...
def main():
    ...
    eval_threshold = checkpoint.get('threshold', 0.5)
    ...
    fallback_mean = np.array(preprocessing_config['fallback_mean']) if 'fallback_mean' in preprocessing_config and preprocessing_config['fallback_mean'] is not None else None
    ...
    results_text = (
        ...
        f"Selected Decision Threshold (from P4): {eval_threshold:.4f}\n"
    ...
    with open("results/tables/cmod_final_results_leakage_free.txt", "w") as f:
    ...
    plt.legend()
    plt.savefig("results/figures/cmod_roc_curve_leakage_free.png")
    plt.close()
    ...
    plt.savefig("results/figures/cmod_confusion_matrix_leakage_free.png")
    plt.close()
